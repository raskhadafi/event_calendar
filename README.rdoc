...
==Install
  ...
  script/plugin install git://github.com/raskhadafi/event_calendar.git

To generate the necessary static files AND the example below:

  rails g event_calendar --example

=== Rails 3

  gem 'event_calendar', :git => 'git://github.com/elevation/event_calendar.git'

* --example: Generate an example model and controller.
...
You can change the default event model name (Event) and controller/view name (Calendar) by passing in two name arguments:
  ...
  rails g event_calendar Model View
...
Unless the --example option is given, these are the only files that will be created.
...
====config/routes.rb
  ...
  match '/calendar/:year/:month', :to => 'calendar#index'

  root :to => 'calendar#index', :defaults => { :year => Time.zone.now.year, :month => Time.zone.now.month }
...
Some helper methods are created, but you could put this in the view. The key is our calendar method, which takes some options.
  ...
  module CalendarHelper
    def month_link(month_date)
      link_to(l(month_date, :format => "%B"), {:month => month_date.month, :year => month_date.year})
    end
    ...
    def event_calendar_options
      {
        ...
        :month_name_text => l(@shown_month, :format => "%B %Y"),
